...
Use the NuGet package name `Rw.Result`:

```bash
dotnet add package Rw.Result
```
...
```xml
<ItemGroup>
  <PackageReference Include="Rw.Result" Version="1.0.0" />
</ItemGroup>
```
...
```text
Rw.Result/
  src/Rw.Result/           # Library project
  tests/Rw.Result.Tests/   # xUnit test project
  LICENSE
  README.md
  Rw.Result.sln
```
